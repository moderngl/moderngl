...
def test_version():
    ...
    match0 = re.search(r'version=\'(\d+\.\d+\.\d+(?:[abc]\d+)?)\'', setup, flags=re.M)

    match1 = re.search(r'__version__ = \'(\d+\.\d+\.\d+(?:[abc]\d+)?)\'', init, flags=re.M)
    match2 = re.search(r'version = \'(\d+\.\d+\.\d+(?:[abc]\d+)?)\'', docs, flags=re.M)
    match3 = re.search(r'release = \'(\d+\.\d+\.\d+(?:[abc]\d+)?)\'', docs, flags=re.M)
