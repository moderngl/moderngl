...
class Context:
    ...
    * The ``factor`` helps you handle sloped geometry (not parallel to near/far plane).
    ...
    def debug_scope(
        ...
        """
        Applies a name to the group of OpenGL API calls that occur within the returned context manager's scope,
        which can be useful in third-party tools like RenderDoc that highlight these scopes.
        Equivalent to ``glPushDebugGroup`` when entering this scope and ``glPopDebugGroup`` when exiting.

        Uses standard OpenGL debug groups if available,
        falling back to GL_EXT_debug_marker if necessary.
        If no debug functionality is available in the context,
        this method's scope has no effect.

        If an exception is raised, the debug group will be popped
        unless the exception originates from pushing the group.

        Args:
            label (str): Should be any name that makes sense to you, e.g. a function name or a summary of the operation
            group_id (int): A numeric ID for the scope. Can be any number; if not provided, autogenerated from the message's hash.
            source (str): "application" or "external"; use "external" if using moderngl as part of a library.

        .. tip::

            This function is unrelated to :py:class:`moderngl.Scope`.
        """
